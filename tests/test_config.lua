Describe("Fixed Jundies presentation", function()
    It("uses the exported hostile plate dimensions", function()
        ...
        ExpectEqual(config.targetHealthScale, 1.1)
        ExpectEqual(config.plateOffsetY, -20)
        ExpectEqual(config.contentPadding, 3)
        ExpectEqual(config.healthTextLeftPadding, 5)
        ExpectEqual(config.healthTextBottomPadding, 1)
        ExpectEqual(config.targetBorderThickness, 1.5)
        ExpectEqual(config.hoverBorderThickness, 1)
        ...
        ExpectEqual(config.debuffScale, 0.8)
        ...
        ExpectEqual(config.colors.debuffTimer[1], 1)
        ExpectEqual(config.colors.debuffTimer[2], 1)
        ExpectEqual(config.colors.debuffTimer[3], 1)
        ...
        ExpectEqual(config.nameGap, 1)
        ExpectEqual(config.nameFontSize, 10)
        ExpectEqual(config.friendlyNameFontScale, 1.65)
        ExpectEqual(config.friendlyFrameRefreshInterval, 0.3)
        ExpectEqual(config.nameFontFlags, "")
        ExpectEqual(config.expresswayFontFlags, "OUTLINE")
        ExpectEqual(config.nameBoldOffset, 0.5)
        ExpectEqual(config.nameOutlineThickness, 1)
